...
class Mecanism():
    def __init__(self):
        ...
        # fizica obiectului
        self.M = 0.5 # mass of the cart in kg
        self.m = 0.01 # mass of the pendulum in kg
        self.b = 0.8 # coeficient of friction for the cart
        self.bp = 0.3 # coeficient of friction for the pendulum
        self.l = 1  # length to pendulum center of the mass
        self.I = 2 # mass momentum of inertia for the pendulum
        self.F = 0.8   # force applied to the cart
        self.x = 0   # cart position coordinate
        self.theta = 0.77* pi # pendulum angle from vertical (down) in radians
        self.dx = 0  # first derivative of x is zero (no speed)
        self.d2x = 3 # second derivative of x is zero (no acceletation)
        self.dtheta = 0.3 # no variation for theta
        self.d2theta = 5 # no acceleration for theta
        self.g = 9.8 # the gravitational acceleration
        
        self.precision = 0

        self.t = self.theta/2 / pi * 90
    ...
    def dynamics(self, F = 0):
        ...
        self.d2theta += -(self.m * self.l * (self.dtheta ** 2) * cos(self.theta) * sin(self.theta))/(self.M + self.m)/2
        self.d2theta += -cos(self.theta) * F / (self.M +self.m)/2
        self.d2theta = self.d2theta / (self.l * (5/3 - (self.m * (cos(self.theta)**2))/(self.M+self.m)))/2
        ...
        self.t = self.t + self.dtheta * self.scaleTime + 0.3 * self.d2theta * (self.scaleTime ** 2)
        ...
        newX = self.x + self.dx * self.scaleTime + 0.3 * self.d2x * (self.scaleTime ** 2)
